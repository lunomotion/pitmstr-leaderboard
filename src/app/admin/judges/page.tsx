...
export default function JudgesPage() {
  ...
  const [records, setRecords] = useState<JudgeRecord[]>([]);
  const [fields, setFields] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/admin/entities?table=Judges")
      .then((res) => res.json())
      .then((json) => {
        if (json.success) {
          ...
          setError(json.error || "Failed to load judges");
        }
      })
  ...
  // Pick the best display columns (skip id, prioritize name-like fields)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {loading
              ? "Loading..."
              : `${records.length} judge${records.length !== 1 ? "s" : ""} registered`}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-slate-500 text-sm">Loading judges...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-16">
            <ClipboardList className="w-12 h-12 text-red-300 mb-3" />
            <p className="text-red-600 font-medium">Error loading judges</p>
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-slate-600 font-medium">
              {search ? "No matching judges" : "No judges registered yet"}
            </p>
          </div>
        ) : (
